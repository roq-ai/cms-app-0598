...
import { ServiceRequestListPage as ShowCaseList } from 'pages/service-requests';

function DashboardPage() {
  const { session } = useSession();

  const serviceRequestStat = useSWR<PaginatedInterface<ServiceRequestInterface>>(
    () => `/service-requests`,
    () => getServiceRequests({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Service Requests',
      result: serviceRequestStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`service-requests`} passHref legacyBehavior>
                  <Button
