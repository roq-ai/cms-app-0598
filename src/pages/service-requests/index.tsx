...
import { getServiceRequests, deleteServiceRequestById } from 'apiSdk/service-requests';
...
export function ServiceRequestListPage(props: ServiceRequestListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getServiceRequests({
        ...
        order: params.order,
        searchTermKeys: ['issue_description.contains', 'status.contains', 'priority.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ServiceRequestInterface>>(
    () => `/service-requests?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    { id: 'issue_description', header: 'Issue Description', accessorKey: 'issue_description' },
